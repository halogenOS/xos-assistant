...
//! records that close 0056's implementation debt — and by the
//! privacy-self-service unit's AC6 pins: the prompt's tool teaching with
//! its verbatim relay, the six decision records, the erasure decision's
//! idempotency refinement, the no-write amendment's second clause, every
//! fixed line verbatim, and the four policy edits with the two assessment
//! notes. Each pin reads the
...
fn the_policy_edits_and_the_two_assessment_notes_ship() {
    let policy = flattened(&repo_file("docs/privacy/privacy-policy.md"));
    assert!(
        policy.contains(
            "Erasure, and the objection described below, work in the chat itself, without \
             writing to us"
        ),
    ...
    assert!(
        policy.contains(
            "direct chats with the assistant are switched off today, so the groups are \
             where these commands are reached"
        ),
        "the in-chat sentence names the group reach under the direct-chat switch"
    );
    assert!(
        policy.contains("One step does run by machine (added 23 August 2026)"),
        "the automated-decision sentence carries its carve-out"
    );
    assert!(
        policy.contains(
            "Updated 23 August 2026: objecting to collection going forward is honored in \
             place"
        ),
        "the objection sentence records the in-place honoring"
    );
    assert!(
        policy.contains("Four things deletion does not reach (the fourth added 23 August 2026)"),
        "the deletion reach list gains the surviving stub"
    );
    assert!(
        policy.contains("remembering that you objected is what it costs"),
