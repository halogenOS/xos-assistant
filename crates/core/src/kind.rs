...
mod tests {
    ...
    /// BLOCKED, 2026-08-28, and left here as the record of it: the
    /// framework began writing a `date_marker` block of its own on every
    /// user-voiced consumer append (agent-ledger a50d046), and no
    /// read-through list in this core names that kind, because every one of
    /// them was written before the kind existed. A marker therefore settles
    /// the owing-tail walk that a run of read-through kinds and erased rows
    /// is meant to be read past — the single cause of the sixty-odd
    /// assertions this workspace's suite fails on today, every one of them
    /// naming that block.
    ///
    /// Which kinds this core reads through is its own decision, reaching
    /// the window, the deletion mirror and every projection, and no spec
    /// records it yet. Naming the marker restores the run, as the first
    /// assertion below shows; adopting that everywhere is a reconciliation
    /// unit's work, not a change unit 32 may make. The second assertion is
    /// the state that unit reaches, and this test goes green the day it
    /// lands — or the day the framework checkout is pinned back.
    #[tokio::test]
    #[ignore = "blocked: this core has no decision yet on the framework's date-marker blocks"]
    async fn a_read_through_run_spans_the_frameworks_date_marker() {
        let store =
            Store::in_memory_with(crate::schema::store_config()).expect("an in-memory store opens");
        let conversation = store
            .create_conversation("p".into(), "m".into(), "M".into(), "v".into())
        ...
        let behind = store
            ...
            .await
            .expect("the text block appends");
        // The user-voiced consumer append is the write path the framework
        // writes its marker on, so the marker lands between the two.
        summoned_message(&store, conversation, "the newest message").await;
        let tx = store.tx();

        assert_eq!(
            newest_block_id_past_erased(&tx, conversation, &[CHAT_MESSAGE_KIND, "date_marker"])
                .await
                .expect("the read naming the marker runs"),
            Some(behind),
            "a read-through list that names the marker spans it"
        );
        assert_eq!(
            newest_block_id_past_erased(&tx, conversation, &[CHAT_MESSAGE_KIND])
                .await
                .expect("the read runs"),
            Some(behind),
            "an unnamed marker settles the walk today; where it stops doing \
             so is the reconciliation's decision, not this unit's"
        );
    }
}
