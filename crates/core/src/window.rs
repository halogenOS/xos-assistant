...
impl ReplyWindow {
    ...
    /// and a change that failed hands the grant back before it is reported,
    /// so a redelivered ask is not silenced by an attempt that delivered
    /// nothing.
    ///
    /// `None` is the exhausted window — the change never ran. `Some(Ok(_))`
    /// is the change's own answer, which spent its grant. `Some(Err(_))` is
    /// the failed change after the hand-back; the caller supplies its own
    /// wording for each.
...
mod tests {
    ...
    /// by an attempt that delivered nothing.
    #[tokio::test(start_paused = true)]
    async fn a_granted_change_spends_and_a_failed_one_hands_the_grant_back() {
        ...
        let granted: Option<Result<&str, &str>> = window.grant_with(1, async { Ok("done") }).await;
        assert_eq!(
            granted,
            Some(Ok("done")),
