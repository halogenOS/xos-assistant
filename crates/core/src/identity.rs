...
//! concludes on a principal's row through this module's own conclusion.
//! Nothing here ever writes into the ledger.
//!
...
//! through it so the display fields stay frozen — after a deletion, no
//! command re-materializes the emptied fields.
...
/// this so the display fields are never refreshed.
///
/// # Errors
///
...
/// else — no display refresh rides along. Returns whether the write changed
...
/// display name to the empty string under the schema's non-null contract,
/// the username to its typed absence — so the flag survives its own
/// person's deletion and collection stays stopped; an unflagged row is
/// deleted whole, as it always was. Emptying an already-empty stub and
...
pub(crate) async fn conclude_erasure(tx: &StoreTx, principal_id: i64) -> Result<(), StoreError> {
    domain_run(tx, DOMAIN, move |conn| {
        let emptied = conn.execute(
            &format!(
                "UPDATE principals SET display_name = '', username = NULL
                 WHERE id = ?1 AND {COLUMN_OPTED_OUT} = 1"
            ),
            [principal_id],
        )?;
...
mod tests {
    ...
    async fn the_standing_reads_and_the_flag_writes_report_honestly() {
        ...
        let id = resolve_principal(&tx, "a".into(), sender("42", "Ada", Some("ada")))
            .await
            .unwrap();
    ...
    /// emptied — the flag standing, the display fields gone — and a repeat
    ...
    async fn a_flagged_principals_erasure_keeps_the_emptied_stub() {
        ...
        let (display_name, username, opted_out): (String, Option<String>, i64) =
            domain_run(&tx, DOMAIN, move |conn| {
                Ok(conn.query_row(
                    &format!(
                        "SELECT display_name, username, {COLUMN_OPTED_OUT}
            ...
            .await
            .unwrap();
        assert_eq!(display_name, "", "the display name empties to the string");
