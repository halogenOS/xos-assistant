...
async fn an_inbound_message_becomes_an_outbound_reply() {
    ...
    // The ledger, block by block: the recorded message, then the answer.
    ...
    assert_eq!(blocks[0].fields["text"], json!("What is the plan?"));
    assert_eq!(blocks[0].fields["authority"], json!("member"));
    assert_eq!(blocks[0].fields["origin"], json!("origin-7"));
    ...
    assert_eq!(blocks[1].role, Some(Role::Assistant));
...
async fn two_channels_stay_two_conversations() {
    ...
    for conversation in &conversations {
        let blocks = support::settle(&fixture.store, conversation.id, "an answered turn", 2).await;
        let text = support::block_text(&blocks[0], "text");
        let other = if text == "the first channel's question" {
            assert_eq!(blocks[0].fields["authority"], json!("member"));
            "the second channel's question"
        } else {
            assert_eq!(text, "the second channel's question");
            assert_eq!(blocks[0].fields["authority"], json!("admin"));
            "the first channel's question"
        };
        assert_eq!(support::block_text(&blocks[1], "content"), answer_to(&text));
        assert!(
            !support::block_text(&blocks[1], "content").contains(other),
...
async fn a_mid_turn_message_is_absorbed_into_the_next_turn() {
    ...
    let blocks = support::settle(&fixture.store, conv, "the settled first turn", 3).await;
    assert_eq!(support::block_text(&blocks[0], "text"), "message one");
    assert_eq!(support::block_text(&blocks[1], "text"), "message two");
    assert_eq!(
    ...
    let blocks = support::settle(&fixture.store, conv, "the settled second turn", 5).await;
...
fn a_restarted_process_answers_a_known_channel() {
    ...
    process_runtime().block_on(async {
        ...
        assert_eq!(
            shape,
            vec![CHAT_MESSAGE_KIND, "text", CHAT_MESSAGE_KIND, "text"],
