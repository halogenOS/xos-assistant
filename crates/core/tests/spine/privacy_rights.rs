...
/// One principal's raw identity row on the suite's adapter: id, display
/// name, username, and the suppression flag — `None` when no row stands.
async fn principal_row(
    store: &Store,
    external_id: &str,
) -> Option<(i64, String, Option<String>, i64)> {
    let external = external_id.to_owned();
    domain_run(&store.tx(), DOMAIN, move |conn| {
        let mut statement = conn.prepare(
            "SELECT id, display_name, username, opted_out FROM principals
...
async fn await_principal_shape(
    ...
    external_id: &str,
    what: &str,
    accept: impl Fn(&Option<(i64, String, Option<String>, i64)>) -> bool,
) {
...
/// message — even one carrying refreshed display fields — leaves no row in
...
async fn a_standing_flag_drops_the_persons_messages_with_no_write() {
    ...
    let (_, display, username, flag) = principal_row(&fixture.store, "A")
        .await
    ...
    // The dropped message carries changed display fields on purpose: a
    ...
    dropped.sender.display_name = "A, renamed".into();
    ...
    let (_, display_after, username_after, _) = principal_row(&fixture.store, "A")
        .await
        .expect("the row still stands");
    assert_eq!(display_after, display, "no identity refresh rode the drop");
    assert_eq!(username_after, username);
    assert_eq!(
        fixture
            .script
            .turns
...
/// keep answering, nothing refreshes the frozen display fields — even when
/// the command itself carries new ones — and `/unblockprivacy` reopens
...
async fn an_opted_out_persons_commands_answer_frozen_and_unblock_reopens() {
    ...
    let (principal_id, frozen_display, frozen_username, _) = principal_row(&fixture.store, "A")
        .await
        .expect("the row stands");

    // The person's own commands, each carrying changed display fields: the
    ...
    for (token, expected) in [
        ...
        message.sender.display_name = "A, renamed".into();
    ...
    let (_, display, username, _) = principal_row(&fixture.store, "A")
        .await
        .expect("the row stands");
    assert_eq!(display, frozen_display, "the display fields stay frozen");
    assert_eq!(username, frozen_username);
    ...
    assert_eq!(
        principal_row(&fixture.store, "A").await.expect("the row").3,
        0,
        "the flag is cleared"
    );
    let mut returned = inbound(&room, ChannelKind::Group, "A", "back again");
    returned.sender.display_name = "A, renamed".into();
    ...
    assert_eq!(
        ...
        "A, renamed",
        "an unflagged person's message refreshes the display fields again"
    );
    assert_eq!(
        command_reply(&fixture.assistant, &room, "A", privacy::OPT_IN_COMMAND).await,
...
async fn erasure_with_the_flag_standing_keeps_the_stub_and_a_repeat_reports_completion() {
    ...
    await_principal_shape(
        ...
        "A",
        "the emptied stub",
        |row| matches!(row, Some((_, display, None, 1)) if display.is_empty()),
    )
    .await;
    assert!(
        stored_texts(&fixture.store, receipt.principal_id)
            ...
            .iter()
            .all(Option::is_none),
        "the texts are nulled around the surviving stub"
    );
...
/// AC1's upgrade pin: a store the previous unit's binary wrote — no
/// suppression flag, the domain's version at eleven — upgrades through the
...
async fn a_version_eleven_store_upgrades_through_the_suppression_step_alone() {
    let db = support::TempDb::new("v11-upgrade");
    {
        let store = Store::open_with(db.path(), store_config()).expect("the store opens");
        domain_run(&store.tx(), DOMAIN, |conn| {
            conn.execute(
                "INSERT INTO principals (adapter, external_id, display_name, username)
                 VALUES ('test-adapter', 'A', 'Sender A', NULL)",
                [],
            )?;
            ...
            assert!(
                refused.is_err(),
                "the genuine version-eleven table has no suppression flag"
            );
            Ok(())
        })
        .await
    ...
    assert_eq!(
        principal_row(&reopened, "A").await.expect("the row").3,
        0,
