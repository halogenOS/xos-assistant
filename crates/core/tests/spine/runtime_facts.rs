...
//! prove is here: the registration, the recorded choice's entry, and the
...
/// calls the tool, the recorded choice resolves it and admission admits the call
...
async fn a_member_reaches_the_tool_and_reads_the_model_the_turn_runs_on() {
    ...
    assert!(
        tool_choice_names(&blocks).contains(&runtime::NAME.to_owned()),
...
async fn the_stated_model_is_the_one_the_conversation_runs_on() {
    ...
    // assemblies on one store would race for the same owed turn, and this
    // test restarts inside one runtime, where a drop would leave the first
    // assembly running and watching the store. The stop is awaited.
