...
| D2 | Identity | Deleted on erasure of the person. Display fields are refreshed from the platform on each later message and never accumulate a history. Qualified 2026-08-23: for a person whose suppression flag stands, the refresh stops with the flag, and erasure empties the row to the flag-bearing remnant described under D9 instead of deleting it — remembering the objection is what honoring it takes. |
