...
| D3 | Circumstance | Arrival time, platform send time, reply reference, whether the message was addressed to the assistant, the authority held in that chat at that moment. Extended 2026-08-31 (unit T3): for an edited message the platform's edit time is what is recorded as its send time — the moment that version came into being — and the arrival time beside it is unchanged. Recorded 2026-09-04 (unit 58): the platform send time is not stored only, it also reaches R1 in every request that carries the message, as R1 states. | Content table of the message block kind |
...
| D9 | Suppression flag (added 2026-08-23) | One boolean on the identity row recording that the person opted out of collection on that platform. Purpose: honoring the objection going forward — from the moment it stands, the person's new messages are dropped at ingestion, except their own privacy commands, which the ingestion exempts from suppression so the objection can be undone from inside, and the flag survives the person's own deletion because forgetting who objected would silently resume collection. Scoped 2026-09-04 (unit 58): the plain-language route reaches the tool only while no flag stands, because a flagged person's ordinary message is dropped before the model sees it. Corrected 2026-09-05 (unit 58): the person's own /privacyout command or their own plain-language ask through the privacy tool sets the flag. Only the person's own /unblockprivacy command clears it. | Identity table, on the person's row |
...
| D12 | Revision reference (added 2026-08-31, unit T3) | The platform identifier of the message an edited message is a further version of, stored on the later version's row. It is the author's own data, the identifier of a message they sent. | Content table of the message block kind, beside D1 |
...
| D14 | Platform message number (added 2026-09-03, unit 58) | The message's own number on the platform, stored on its row and carried on the message's projected line, where sections 6 and 9 already state that it rides in every request that carries the message and that erasure nulls it. It names a message and not a person, and it is the author's own data, the identifier of a message they sent. | Content table of the message block kind, beside D1 |
...
emptied in every conversation (widened 2026-09-03, unit 58), their direct conversations are
