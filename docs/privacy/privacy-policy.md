...
A data protection officer has not been appointed, as the statutory thresholds of § 38 BDSG
are not met.
...
not stored at all, because the platform does not disclose who wrote them.

Messages you send to the assistant in a direct chat are stored the same way.

_Purpose:_ answering questions in the group, and reading enough of the conversation to
answer in context.
...
one member withdrawing it could not stop a group conversation from being stored.
...
Stored alongside: when the message arrived, which message it replied to, whether it was
addressed to the assistant, and whether you were an administrator of that group at the
time.

_Purpose:_ attributing messages inside a conversation, addressing you in an answer, and
making deletion on request possible at all.
...
processor. Two things leave our server:
...
Your display name and your numeric account identifier stay with us and are not sent.
Nothing else about you is attached, and nothing is sent that the group did not already see.
...
|-----------|------|-------|
| Requesty Inc. | Passes the conversation to a language model and returns the answer. Requests enter the EU region in Frankfurt, Germany, and are served on infrastructure in the EU. Zero data retention is configured: nothing is stored once the answer has been returned, and nothing is used for training. | Processor, Article 28 GDPR |
| Sub-processors engaged by Requesty | Requesty engages the model providers it routes to as its own sub-processors, under its own agreements, and stays answerable to us for them. | Sub-processors, Article 28(2) and (4) GDPR |
...
No data is transferred to a third country outside the EU/EEA. Messages are stored on a
server in Germany, and requests to the language model stay in the EU. Were routing outside
the EU ever to occur, the agreement with our processor carries the European Commission's
standard contractual clauses as the safeguard under Article 46 GDPR.
...
deletion on request, and that is fast and complete.

Deleting a message in your chat app does not reach us. The platform does not tell the
assistant, so our copy stays until you ask.

**Deletion, and what it does**

Ask, and all of it goes: the text of every message you wrote, the time and the reply
reference stored with it, your whole direct conversation with the assistant, and the
identity data described above. What remains is an empty placeholder holding a position in
...
(Article 17), to restriction of processing (Article 18), to data portability (Article 20),
and to object to processing based on legitimate interests (Article 21). Everything
described here rests on legitimate interest, so the right to object covers all of it.

To exercise any of these, write to
[privacy@halogenos.org](mailto:privacy@halogenos.org). Tell us your username and the group,
so that your data can be found. Requests are free and are answered within one month. Every
request is answered by a person, and no decision here is automated.
