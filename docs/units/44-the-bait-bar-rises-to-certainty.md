...
teaching change plus its pins; no mechanism moves.
...
- **The join report requires certainty, in the teaching's own words, 2026-08-30.**
  ...
  the same evidentiary bar. *Rejected:* a mechanism (a name classifier, a pattern
...
- **AC1** The amended span is pinned byte for byte; the pre-existing join-rule
  fact pins (`teaching.rs:662-676`) are updated to the new sentences and pass; no
  other teaching text changes (the composed prompt diff is exactly this span).
...
- Deploy-relevant: rides the next deploy with unit 42.
