...
READ-ONLY identity path: the username is not refreshed. The freeze
the stub promises holds even across the person's own commands, and after a
deletion no command re-materializes the emptied field.
...
- **Refreshing identity on exempt commands.** The freeze would leak — every
  command would re-collect the username the stub exists to stop
